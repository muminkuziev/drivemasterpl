...
async function main() {
  ...
  for (const category of CATEGORIES) {
    const filePath = path.join(__dirname, 'signs-data', `actions-${category}.json`);
    ...
    let updated = 0;
    let missing = 0;
    for (const { code, actionUz } of entries) {
  ...
  const stillEmpty = await p.roadSign.count({ where: { actionUz: null } });
  console.log(`Remaining signs with no actionUz: ${stillEmpty}`);
